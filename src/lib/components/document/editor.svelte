<script lang="ts">
    ...
    */
    ...
    //    {   caption: 'Comment',      description: 'With this you can comment the above paragraph',   icon: FaComment,                    on_choice: (range) => { if(range) editor.chain().focus().deleteRange(range).setAsComment().run(); else editor.chain().focus().setAsComment().run() }, is_active: () => editor?.isActive('CommentBlock')  } ,
        {   caption: i18n({en: 'Quote', es: 'Cita', pl: 'Cytat'}),        description: 'To quote someone',                         tags: 'quote',       mricon: 'text-quote',                 on_choice: (range) => { if(range) editor.chain().focus().deleteRange(range).setAsQuote().run(); else editor.chain().focus().setAsQuote().run() }, is_active: () => editor?.isActive('QuoteBlock')  } ,
    ...
    //    {   caption: 'Info',         description: 'An important info about above paragraph',         icon: FaInfo,                       on_choice: (range) => { if(range) editor.chain().focus().deleteRange(range).setAsInfo().run(); else editor.chain().focus().setAsInfo().run() }, is_active: () => editor?.isActive('InfoBlock')  },
        {   caption: i18n({en: 'BulletList', es: 'Lista con viñetas', pl: 'Lista punktowana'}),  description: 'Unordered list of items',   tags: 'bullet', mricon: 'list',                     on_choice: (range) => { if(range) editor.chain().focus().deleteRange(range).toggleBulletList().run(); else editor.chain().focus().toggleBulletList().run() }, is_active: () => editor?.isActive('bulletList')  },
    ]

    const paletteInsertCommands = () => [
        {   caption: i18n({en: 'Image', es: 'Imagen', pl: 'Obraz'}),        description: 'Add image to document',           tags:'img,picture',      mricon: 'image',                       on_choice: (range) => { if(range) editor.chain().focus().deleteRange(range).run(); if(onAddImage) onAddImage(onAddedImageReady);  } } ,
        {   caption: i18n({en: 'Table', es: 'Tabla', pl: 'Tabela'}),        description: 'Table',                           tags: 'table',          mricon: 'table',                      on_choice: (range) => { if(range) editor.chain().focus().deleteRange(range).insertTable().run(); else editor.chain().focus().insertTable().run() }, is_active: () => editor?.isActive('table')  },
        {   caption: i18n({en: 'Horizontal rule', es: 'Regla horizontal', pl: 'Pozioma linia'}), description: 'Add horizonal role',           tags: 'hr,line',  mricon: 'minus',                       on_choice: (range) => { if(range) editor.chain().focus().deleteRange(range).setHorizontalRule().run(); else editor.chain().focus().setHorizontalRule().run();  } }
    ]
